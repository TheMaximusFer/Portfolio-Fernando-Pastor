...
def joinFactors(factors: ValuesView[Factor]):
    ...
    "*** YOUR CODE HERE ***"
     # Calcular las variables no condicionadas y condicionadas del factor resultado.
    unconditioned = set()
    conditioned = set()

    for factor in factors_list:
        unconditioned = unconditioned.union(set(factor.unconditionedVariables()))
        conditioned = conditioned.union(set(factor.conditionedVariables()))

    # Si una variable es no condicionada en algún factor, no puede seguir como condicionada (usamos la diferencia de conjuntos).
    conditioned = conditioned - unconditioned

    # Crear el nuevo factor resultado del join.
    joinedFactor = Factor(unconditioned, conditioned, variableDomainsDict)

    # Para cada asignación posible, multiplicar las probabilidades de todos los factores
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        "*** YOUR CODE HERE ***"
         # Calcular nuevas variables no condicionadas y condicionadas
        newUnconditioned = set(factor.unconditionedVariables())
        newUnconditioned.remove(eliminationVariable)   # eliminamos la variable a eliminar
        newConditioned = set(factor.conditionedVariables())

        #  Crear el nuevo factor resultado de la eliminación
        variableDomainsDict = factor.variableDomainsDict()
        newFactor = Factor(newUnconditioned, newConditioned, variableDomainsDict)

        # Para cada asignación de las variables del nuevo factor, sumamos sobre todos los valores posibles de la variable eliminada
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            probTotal = 0.0
            # Sumamos sobre todos los posibles valores de la variable que eliminamos.
            for elimValue in variableDomainsDict[eliminationVariable]:
                ...
                probTotal += factor.getProbability(extendedAssignment)
            
            newFactor.setProbability(assignment, probTotal)
