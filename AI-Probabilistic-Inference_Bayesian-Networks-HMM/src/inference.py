...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # Construyendo redes bayesianas: lista de variables
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    # Construyendo redes bayesianas, lista de aristas: (x, y) significa arista de x a y
    edges = [(PAC, OBS0), (PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS1)]
    # Construir el dominio para cada variable (tipo lista)
    variableDomainsDict = {}
    ...
    """ 
    manhattanDistance(...): Calcula la distancia más larga posible en el mapa.
    + MAX_NOISE: Añade el margen de error del sensor.
    + 1: Para incluir el último número del intervalo.
    """
    variableDomainsDict[OBS0] = [i for i in range(manhattanDistance((0,0),(X_RANGE-1,Y_RANGE-1)) + MAX_NOISE + 1)]
    variableDomainsDict[OBS1] = [i for i in range(manhattanDistance((0,0),(X_RANGE-1,Y_RANGE-1)) + MAX_NOISE + 1)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Para cada variable del orden de eliminación:
        #    - unir todos los factores que la contengan (joinFactorsByVariable)
        #    - eliminar esa variable del factor unido (eliminate),
        #      salvo que el factor tenga solo una variable no condicionada,
        #      en cuyo caso se descarta directamente.
        for elimVar in eliminationOrder:
            # Unimos todos los factores que contienen elimVar
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, elimVar)

            # Si por lo que sea no hay factor que la contenga, seguimos
            if joinedFactor is None:
                continue

            # Si el factor resultante tiene solo una variable no condicionada, y precisamente es la que queremos eliminar, lo descartamos.
            if len(joinedFactor.unconditionedVariables()) == 1:
                # No lo añadimos de nuevo a la lista.
                continue

            # Si tiene más de una variable no condicionada, eliminamos elimVar
            eliminatedFactor = eliminate(joinedFactor, elimVar)
            # Añadimos el factor resultante a la lista de factores actuales
            currentFactorsList.append(eliminatedFactor)

        # Cuando ya hemos eliminado todas las variables de eliminationOrder, unimos todos los factores restantes en uno solo.
        finalFactor = joinFactors(currentFactorsList)

        # Normalizamos para obtener P(queryVariables|evidenceDict)
        resultFactor = normalize(finalFactor)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Suma total de todos los valores de la distribución
        total = float(sum(self.values()))

        # Si la suma es 0, no hacemos nada (no se puede normalizar)
        if total == 0:
            return

        # Dividimos cada valor por la suma total para que la suma sea 1
    ...
    def sample(self):
        ...
        # Si la suma es 0, no hacemos nada (no se puede samplear).
        if total == 0:
            return None

        # Elegimos un número aleatorio entre 0 y total.
        r = random.random() * total

        # Recorremos acumulando hasta sobrepasar r.
        acumulado = 0.0
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Caso el fantasma está en la cárcel:
        if ghostPosition == jailPosition:
            # Si el sensor dice None --> prob = 1
            if noisyDistance is None:
                return 1.0
            # Si da una distancia numérica imposible
            return 0.0

        # Caso del fantasma no está en la cárcel y la observación es None:
        if noisyDistance is None:
            return 0.0


        # Caso general: fantasma libre y lectura numérica.
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
         # Posición actual de Pacman y posición de la cárcel del fantasma
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()

        # Actualizar las creencias multiplicando por P(observation|posición)
