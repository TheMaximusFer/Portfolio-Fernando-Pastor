...
int initsocket(struct addrinfo *servinfo, char f_verbose)
{
    ...
    {   // bucle que recorre la lista de direcciones
        ...
        {   // socket creado correctamente
            ...
            if (connect(sock, servinfo->ai_addr, servinfo->ai_addrlen) < 0)                                                                             //10, 11, 12
            {
...
int main(int argc, char * argv[])
{
    ...
    while ((len = read(0, msg, MAX_BUFF_SIZE)) > 0)
    {
        ...
        if ((sentbytes = send(sock, msg, len, 0)) < 0)                                                                                                  //13, 14, 15
        {
    ...
    if (shutdown(sock, SHUT_WR) < 0) {                                                                                                                  //16, 17
    ...
    if ((recvbytes = recv(sock, &num, sizeof(num), 0)) != sizeof num)                                                                                   //18, 19, 20
    {
