...
int establecer_servicio(struct addrinfo *servinfo, char f_verbose)
{
    ...
    sock = socket(servinfo->ai_family, servinfo->ai_socktype, servinfo->ai_protocol);                                                                   //23, 24, 25
    if (sock < 0)
    {
    ...
    if (bind(sock, servinfo->ai_addr, servinfo->ai_addrlen) < 0)                                                                                        //26, 27, 28
    {
...
int main(int argc, char * argv[])
{
    ...
    int sock;                   // descriptore de socket
    ...
    const char fin = 4; // carácter ASCII end of transmission (EOT)
    ...
    while (1)
    {
        ...
        num = 0;
        do {
            if ((readbytes = recvfrom(sock, &msg, BUFF_SIZE, 0, (struct sockaddr *)&caddr, &clen)) < 0)                                                                                         //31, 32
            {
        ...
        } while (readbytes > 0 && (msg[readbytes-1] != fin));
        ...
        if (sendto(sock, &netNum, sizeof netNum, 0, (struct sockaddr*) &caddr, clen) < 0)                                                                                                   //33
        {
