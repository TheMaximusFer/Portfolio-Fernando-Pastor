...
import { postWorld, postMarker, getUsernameFromToken } from "../services/apiService.js";
...
export default function WorldCreate() {
  ...
  const filteredSuggestions = ALL_TAGS
    .filter(
      (t) =>
  ...
  const removeMarkerRow = (idx) => {
    setMarkers((prev) => {
      ...
      if (victim?.photoUrl?.startsWith("blob:")) URL.revokeObjectURL(victim.photoUrl);
      const next = [...prev];
      next.splice(idx, 1);
      return next;
    });
  ...
  const handleMarkerPhotoSelect = (idx, files) => {
    ...
    setMarkers((prev) => {
      const next = [...prev];
      const old = next[idx];
      if (old.photoUrl) URL.revokeObjectURL(old.photoUrl);
      next[idx] = { ...old, photoUrl: URL.createObjectURL(file) };
      return next;
    });
  };
  ...
  const addToGallery = (list) => {
    ...
    setThumbs((prev) => [
      ...prev,
      ...files.map((f) => ({ id: crypto.randomUUID(), url: URL.createObjectURL(f) })),
    ]);
  };
  const delThumb = (id) => {
    setThumbs((prev) => {
  ...
  useEffect(() => {
    return () => {
      ...
      thumbs.forEach((t) => t.url?.startsWith("blob:") && URL.revokeObjectURL(t.url));
    };
  }, []); // solo al desmontar
    ...
    const worldDTO = {
      ...
      version: mcVersion,
      public: isPublic,
      // Ojo: aquí mandamos las URLs blob, el backend las verá como strings
      images: thumbs.map((t) => t.url),
    };
    ...
    <div className="uploadpage">
      ...
      <div className="form-shell">
        <div className="form-title-row">
          <h1 className="form-title">SUBIR MUNDO</h1>
        </div>
        ...
        <div className="form-grid">
          ...
          <div className="toggle-group">
            ...
            <div className="platform-toggle" role="tablist" aria-label="Plataforma">
              <button
                type="button"
              ...
              <button
                type="button"
                className={`plat-btn ${platform === "Bedrock" ? "active" : ""}`}
            ...
            <div className="row row--grow">
              ...
              <div className="markers-panel">
                  ...
                  <article key={m.id} className="marker-card">
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => handleMarkerPhotoSelect(i, e.target.files)}
                      />
                      <label
                        htmlFor={`mphoto-${m.id}`}
                        className={`marker-upload-box ${m.photoUrl ? "has-image" : ""}`}
                    ...
                    <div>
                      <div className="marker-main">
                        <div style={{ width: "100%" }}>
                          <span className="mini-label">NOMBRE DEL MARCADOR</span>
                          <div className="fieldline">
                            <input
                              className="input"
                              value={m.nombre}
                              onChange={(e) => updateMarker(i, "nombre", e.target.value)}
                      ...
                      {/* Dimensión */}
                      <div className="dimension-toggle" role="tablist" aria-label="Dimensión">
                        <button
                          type="button"
                          className={`plat-btn ${m.dimension === "Overworld" ? "active" : ""}`}
                          onClick={() => updateMarker(i, "dimension", "Overworld")}
                        ...
                        <button
                          type="button"
                          className={`plat-btn ${m.dimension === "Nether" ? "active" : ""}`}
                        ...
                        <button
                          type="button"
                          className={`plat-btn ${m.dimension === "End" ? "active" : ""}`}
                ...
                {/* Añadir */}
                <button className="marker-add" type="button" onClick={addMarker}>
              ...
              >
                <div className="gallery-drop-inner">
                  ...
                  <div className="gallery-hint">
                    ...
                    <br />
                    O PULSA EL RECUADRO PARA SUBIR
                  </div>
                </div>
                <input
                  ref={galleryInputRef}
                ...
                <div className="thumbs-grid">
                  {thumbs.map((t) => (
                    <div key={t.id} className="thumb-item">
                      <img className="thumb-img" src={t.url} alt="captura" />
                      <button
                        className="thumb-del"
                        type="button"
