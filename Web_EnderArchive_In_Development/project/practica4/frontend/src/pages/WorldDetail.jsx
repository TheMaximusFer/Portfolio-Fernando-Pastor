...
import { fetchUserMaps, fetchMarkers, postReport, postFavourite, fetchAllMyFavourites, deleteFavourite } from "../services/apiService.js";
...
export default function WorldDetail() {
            ...
            try {
                ...
                if (token){
                  ...
                  const existe = favourites.some(item => {
                    ...
                    return itemName.toLowerCase() === worldName.toLowerCase() && 
  ...
  const images = [
    "https://picsum.photos/seed/morbi1/1200/900",
    "https://picsum.photos/seed/morbi2/1200/900",
    "https://picsum.photos/seed/morbi3/1200/900",
    "https://picsum.photos/seed/morbi4/1200/900",
    "https://picsum.photos/seed/morbi5/1200/900",
    "https://picsum.photos/seed/morbi6/1200/900",
  ];
  ...
  if (isLoading) {
      return (
          <div className="loading-page">
              <Header />
              <p>Cargando detalles de **{worldName}** de **{worldOwner}**...</p>
          </div>
      );
  }
  ...
  if (!worldData) {
      return <div className="error-page">Mundo no encontrado o datos incompletos.</div>;
  }
            ...
            <div className="window-grid">
                    ...
                    <h1 className="window-title">
                        ...
                        <span className="by">
                          ...
                          <span 
                            ...
                            onClick={() => navigate(`/user-profile/${worldData.id.worldOwner}`)}
                        ...
                        {markers.map((m) => (
                            <article key={m.id} className="marker-row">
                                ...
                                <div className="coords">
                                    ...
                                    <div className="coords-line"><span>X</span> {m.x}</div>
                                    <div className="coords-line"><span>Y</span> {m.y}</div>
                                    <div className="coords-line"><span>Z</span> {m.z}</div>
                                </div>
                            </article>
                        ))}
                    </div>
                ...
                <aside className="window-right">
                    ...
                    <div className="gallery-box">
                        <img className="gallery-img" src={images[id]} alt={`Captura ${id + 1}`} />
                    </div>

                    {/* Control de la galería */}
                    <div className="gallery-controls">
                        {/* Botón izquierda */}
                        <button className="square-btn" onClick={prev} aria-label="Anterior"><ArrowLeft /></button>
                        {/* Tantos botones como imágenes tenga el mundo */}
                        <div className="dots">
                            {images.map((_, i) => (
                                <button key={i} className={`dot ${i === id ? "active" : ""}`} onClick={() => setId(i)} aria-label={`Ir a la imagen ${i + 1}`}/>
                            ))}
                        </div>
                        {/* Botón derecha */}
                        <button className="square-btn" onClick={next} aria-label="Siguiente"><ArrowRight /></button>
                    </div>
